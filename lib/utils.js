...
export async function getLastRun(key) {
  const r = getRedis();
  const timestamp = await r.get(key);
  return timestamp || null;
}

export async function setLastRun(key, timestamp) {
  const r = getRedis();
  await r.set(key, timestamp);
}

export function filterNewProposals(proposals, lastRun, dateField) {
  if (!lastRun) return proposals;
  const lastRunDate = new Date(lastRun);
  return proposals.filter(p => {
    const proposalDate = p[dateField] ? new Date(p[dateField]) : null;
    return proposalDate && proposalDate > lastRunDate;
  });
}
